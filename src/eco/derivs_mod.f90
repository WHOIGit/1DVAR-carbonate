...
! map bioparams to local copies
    ...
    ! SP derivs
    dydtt(iSPc) = (growSPc - excrSP_1c - excrSP_2c - grazSPc - pomSPc - respSP)/ SecPerDay
    ...
    ! New TR
    dydtt(iTRc) = (growTRc - excrTR_1c - excrTR_2c - grazTRc - pomTRc - respTR)/ SecPerDay
    ...
    ! New UN
    dydtt(iUNc) = (growUNc - excrUN_1c - excrUN_2c - grazUNc - pomUNc - respUN)/ SecPerDay
    ...
    ! 2. respiration
     !! [HK] write error log (floating overflow error)
     !! write(100, '(6es18.10)') b_BAresp, growBAc, growBAldoc, growBAsdoc, ALC, ASC
...
!                                   * EXP(3*(y(iMZp)/y(iMZc)/q_MZ_p-c1))
    ...
    dydtt(iDETc) = (pomSPc + pomTRc + pomUNc + pomPRTc + pomMZc + pomHZc - disDETc)/ SecPerDay
    dydtt(iDETn) = (pomSPn + pomTRn + pomUNn + pomPRTn + pomMZn + pomHZn - disDETn)/ SecPerDay
    dydtt(iDETp) = (pomSPp + pomTRp + pomUNp + pomPRTp + pomMZp + pomHZp - disDETp)/ SecPerDay
