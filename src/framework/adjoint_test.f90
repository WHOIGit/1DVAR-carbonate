...
!---------------------------------------------------------------------------
  ...
  delta=1d-3
  normg = sqrt(sum(adoptparams0*adoptparams0))
  write (6,*) 'adjoint_test:: normg ',normg
  do while (delta > 1d-11)
     optparams = optparams0-delta*adoptparams0/normg
     indic=4
     call simul(indic,nparams_opt,optparams,fcost,adoptparams,izs,rzs,dzs)
     ! print out diagnostic based on deltaJ = dJ/dp deltap
     write (6,*) 'adjoint_test:: d, dJ:d, J',delta, &
          (fcost-fcost0)/(-delta*sum(adoptparams0*adoptparams0)/normg),fcost
     delta = delta * 0.1
  end do
